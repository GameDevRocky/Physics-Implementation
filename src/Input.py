...
class KeyBoard:
    ...
    def update(self, event):
        if self.active:
            ...
            self.PlayerInput = {
                "movement_x": (self.rightKey.performed - self.leftKey.performed) or (self.D_Key.performed - self.A_Key.performed),
            ...
            self.EditorInput = {
                ...
                "erase": (self.leftMouse.started  or self.leftMouse.performed )and self.Shift_key.performed,
                "paint": self.leftMouse.performed and not self.Shift_key.performed,
                "auto_tile": self.leftMouse.cancelled or self.rightMouse.cancelled,
                "mouse": self.mouse,
                'pressed': self.leftMouse.started and not self.Shift_key.performed,
                'erase_start' : self.leftMouse.started and self.Shift_key.performed,
                'alt': self.Shift_key.performed,
                'rotate': self.spaceKey.cancelled 
            }
    def clear(self):
        for button in self.buttons:
...
class EditorActions:
    ...
    def update(self, currentDevice, event):
        if self.active:
            ...
            self.erase_start = currentDevice.EditorInput['erase_start']
            self.rotate = currentDevice.EditorInput['rotate']
